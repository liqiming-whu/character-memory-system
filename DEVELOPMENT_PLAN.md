...
# 13. 记忆过多时的展示与管理方案（**已按决策落地，2026-08-09 关闭**）

> 双端方案（CMS/CME 同步入档）。解决记忆/信息数量增长后的展示与管理：不丢数据、不卡 UI、可全量搜索、可批量管理。
> **关闭原因（2026-08-09 用户决策）**：手机端插件受平台性能限制，不应负责大规模展示记忆与增删查改。不做分页/批量管理，统一所有条目最多显示时间最近的 100 条、按从新到旧排序（CMS v2.3.4 / CME v2.4.6 已落地）。数据本身不删除（仅展示截断），仍可通过注入/分析链路完整使用；「分页/懒渲染」等待 Operit 平台支持差异更新后再评估。

## 13.1 现状与问题（双端一致）

| 项 | 现状 | 问题 |
|---|---|---|
| 加载 | `limit=100` 一次性加载（CMS `load_memories` / CME `list_memories`） | 超过 100 条的记忆静默不可见，无任何提示 |
| 展示 | 知识页信息+记忆两栏全量渲染（Compose DSL 节点随条数线性增长） | 长列表渲染性能下降 |
| 搜索 | 本地 `filter`（仅已加载的 100 条范围内） | 搜索不到更早记忆 |
| 管理 | 逐条删除（CMS 两段式 / CME 单击） | 大量清理时只能逐条删，无批量、无容量提示 |

## 13.2 方案设计

**A. 展示层**

1. 分页/加载更多：列表底部「加载更多」+ 增量拉取（每批 50 条）。CMS 原生 `load_memories` 需先验证分页能力（offset/游标）；不支持则按时间倒序分批拉取或明确提示上限。CME `list_memories` 走 SQLite，天然支持 OFFSET/LIMIT。
2. 分组折叠：信息/记忆按类别分组，默认展开前 N 条，可折叠收起。
3. 长列表渲染优化：懒渲染（仅渲染可视区域附近条目），避免 Compose DSL 节点爆炸。
4. 数量提示：统计卡显示「已加载 x / 共 y」；有更多时提示「还有更多，点击加载」。

**B. 管理层**

5. 搜索升级：输入搜索词走后端全量检索（CMS `load_memories` 的 `query` 参数 / CME `list_memories` 的 `query`），替代本地 filter。
6. 批量删除：多选 → 批量删除。CME（SQLite）批量删易实现、优先做；CMS（原生 `delete_memory` 逐条）串行批量。
7. 容量感知：显示记忆总数、最近更新时间；超阈值提示建议清理，可引导先 `export_backup` 备份再清。

## 13.3 双端差异与落地顺序

- **CME（SQLite）**：分页 + 批量删 + 后端检索能力最强，优先落地。
- **CMS（原生向量库）**：先验证 `load_memories` 分页能力，搜索走后端 query。
- 落地顺序：① 分页/加载更多 → ② 搜索走后端全量 → ③ 批量删除 → ④ 性能验证（500+/1000+ 条记忆下 UI 流畅度与内存）。
