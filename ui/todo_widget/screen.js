...
function Screen(ctx) {
  ...
  items.push(UI.Row({ fillMaxWidth: true, verticalAlignment: 'center', horizontalArrangement: 'spaceBetween' }, [
    UI.Row({ verticalAlignment: 'center' }, [
      UI.Icon({ name: 'checklist', tint: colors.error, size: 20 }),
      UI.Spacer({ width: 6 }),
      UI.Text({ text: '待办事项', style: 'titleSmall', fontWeight: 'bold', color: colors.error }),
    ]),
    UI.Surface({ shape: { cornerRadius: 8 }, containerColor: colors.errorContainer, padding: { left: 8, right: 8, top: 2, bottom: 2 } }, [
      UI.Text({ text: String(pendingCount) + ' 项', style: 'labelSmall', color: colors.error, fontWeight: 'bold' }),
    ]),
  ]));
  ...
  if (pendingCount === 0 && totalCount === 0) {
    items.push(UI.Column({ horizontalAlignment: 'center', fillMaxWidth: true, padding: 16 }, [
      UI.Text({ text: '暂无待办数据', style: 'bodySmall', color: colors.outline }),
      UI.Text({ text: '请先打开侧边栏加载', style: 'labelSmall', color: colors.outlineVariant }),
    ]));
  } else if (pendingCount === 0) {
    items.push(UI.Column({ horizontalAlignment: 'center', fillMaxWidth: true, padding: 16 }, [
      UI.Icon({ name: 'check_circle', tint: colors.primary, size: 28 }),
      UI.Spacer({ height: 6 }),
      UI.Text({ text: '全部完成！', style: 'bodySmall', color: colors.primary, fontWeight: 'bold' }),
    ]));
  } else {
    ...
    items.push(UI.Column({ fillMaxWidth: true, spacing: 4 }, [
      UI.Row({ fillMaxWidth: true, verticalAlignment: 'center' }, [
        ...
        UI.Text({ text: String(doneCount), style: 'labelMedium', color: colors.primary, fontWeight: 'bold' }),
        ...
        UI.Spacer({ width: 8 }),
        UI.Text({ text: String(pct) + '%', style: 'labelSmall', color: colors.primary }),
      ]),
  ...
  if (lastUpdate) {
    ...
    items.push(UI.Text({ text: '更新: ' + lastUpdate, style: 'labelSmall', color: colors.outlineVariant, fontSize: 9 }));
  }

  items.push(UI.Spacer({ height: 4 }));
  items.push(UI.Surface({ fillMaxWidth: true, shape: { cornerRadius: 8 }, containerColor: colors.primary, alpha: 0.1, padding: { left: 10, right: 10, top: 6, bottom: 6 }, onClick: function() { ctx.navigate('toolpkg:com.operit.character_memory_system:ui:memory_system_ui', {}); }, key: 'openFull' }, [
    UI.Row({ fillMaxWidth: true, horizontalArrangement: 'center', verticalAlignment: 'center' }, [
      UI.Text({ text: '打开完整页面', style: 'labelSmall', color: colors.primary, fontWeight: 'bold' }),
    ]),
  ]));
